...
def tau_CE(lam):
    ...
    def tau_CE_single(lam):
        """Scalar version: returns single threshold value."""
        log2 = np.log(2)
        if lam >= log2:
            return 0.5  # Always abstain when cost exceeds max entropy
        # Solve H(c) = lambda for c in (0.5, 1)
        try:
            return brentq(lambda c: H(c) - lam, 0.5 + 1e-9, 1.0 - 1e-9)
        except ValueError:
            # If brentq fails, return midpoint (shouldn't happen for valid lambda)
            return 0.75
    
    # Handle scalar vs array input like tau_U and tau_B
    scalar = np.isscalar(lam)
    lam = np.atleast_1d(np.asarray(lam, dtype=float))
    ...
    # Vectorize over array elements
    out = np.array([tau_CE_single(l) for l in lam])
    
    return float(out[0]) if scalar else out


# Vectorized version for numpy array operations
tau_CE_vec = np.vectorize(lambda lam: tau_CE(lam), otypes=[float])
